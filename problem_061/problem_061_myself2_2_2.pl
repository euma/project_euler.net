...
for (my $i=$start_num[2];$i<=$end_num[2];$i++)
{
  my $n5n=union_function($i,2);
  $n5n=~/(\d\d)(\d\d)/;
  my $n5a=$1;
  my $n5b=$2;
  for (my $j=$start_num[1];$j<=$end_num[1];$j++) {
    my $n4n=union_function($j,1);
    $n4n=~/(\d\d)(\d\d)/;
    my $n4a=$1;
    my $n4b=$2;
    if ($n5b == $n4a)
    {
      for (my $k=$start_num[0];$k<=$end_num[0];$k++) {
	my $n3n=union_function($k,0);
	$n3n=~/(\d\d)(\d\d)/;
	my $n3a=$1;
	my $n3b=$2;
	if ($n4b == $n3a and $n3b == $n5a)
	{
	  print "$n5n $n4n $n3n\n";
	}
      }
