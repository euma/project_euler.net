...
if ($num == int($num)) { $start6=$num; }
else { $start6=int($num)+1; }
my $end6=0;
$num=sqrt(8*$end+1)/4+1/4;
if ($num == int($num)) { $end6=$num; }
else { $end6=int($num); }

my $start7=0;
$num=sqrt(40*$start/9+1)*3/10+3/10;
if ($num == int($num)) { $start7=$num; }
else { $start7=int($num)+1; }
my $end7=0;
$num=sqrt(40*$end/9+1)*3/10+3/10;
if ($num == int($num)) { $end7=$num; }
else { $end7=int($num); }

my $start8=0;
$num=sqrt(3*$start+1)/3+1/3;
if ($num == int($num)) { $start8=$num; }
else { $start8=int($num)+1; }
my $end8=0;
$num=sqrt(3*$end+1)/3+1/3;
if ($num == int($num)) { $end8=$num; }
else { $end8=int($num); }
sub union_function
{
  my $i=shift;
...
my @start_num=($start3,$start4,$start5,$start6,$start7,$start8);
my @end_num=($end3,$end4,$end5,$end6,$end7,$end8);
...
while (my @res = $p->next)
{
  ...
  for (my $i=$start_num[$p1];$i<=$end_num[$p1];$i++)
  {
	...
	for (my $k=$start_num[$p3];$k<=$end_num[$p3];$k++) {
	  ...
	  if ($n2b == $n3a)
	  {
	    for (my $l=$start_num[$p4];$l<=$end_num[$p4];$l++) {
	      my $n4n=union_function($l,$p4);
	      $n4n=~/(\d\d)(\d\d)/;
	      my $n4a=$1;
	      my $n4b=$2;
	      if ($n3b == $n4a)
	      {
		for (my $m=$start_num[$p5];$m<=$end_num[$p5];$m++) {
		  my $n5n=union_function($m,$p5);
		  $n5n=~/(\d\d)(\d\d)/;
		  my $n5a=$1;
		  my $n5b=$2;
		  if ($n4b == $n5a)
		  {
		    for (my $n=$start_num[$p6];$n<=$end_num[$p6];$n++) {
		      my $n6n=union_function($n,$p6);
		      $n6n=~/(\d\d)(\d\d)/;
		      my $n6a=$1;
		      my $n6b=$2;
		      if ($n5b == $n6a and $n6b == $n1a)
		      {
			print "$n1n $n2n $n3n $n4n $n5n $n6n\n";
			print $n1n+$n2n+$n3n+$n4n+$n5n+$n6n;
			print "\n";
			exit 0;
		      }
