...
#set two chance card
my @chance_card=();
$chance_card[0]=int(rand($pile_size));
$chance_card[1]=$chance_card[0]+int(rand($pile_size-1))+1;
if ($chance_card[1]>$pile_size) {
  $chance_card[1]=$chance_card[1]-$pile_size
}
print "$chance_card[0], $chance_card[1]";

#shuffle community chest card pile
my @community_chest_card_pile=();
for (my $i=0; $i<$pile_size;$i++) {
...
#set ten chance card
#this method will get repeated card?So this method is not ok!
#my @community_chest_card=();
#my $community_chest_card_size=10;
#$community_chest_card[0]=int(rand($pile_size));
#for (my $i=1;$i<$community_chest_card_size;$i++) {
#  $community_chest_card[$i]=$community_chest_card[$i-1]+int(rand($pile_size-1))+1;
#  if ($community_chest_card[$i]>$pile_size) {
#    $community_chest_card[$i]=$community_chest_card[$i]-$pile_size
#  }
#}
#print " @community_chest_card\n";
