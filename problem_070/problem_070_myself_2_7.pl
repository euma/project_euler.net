...
my $p_max=sqrt($target)+1;
my $loop_max=int(($p_max-1)/2)+2;
for (my $i = 1;$i<=$loop_max;$i++) {
  if (not $sieve[$i]) {
    my $prime_i=2*$i+1;
    print "$prime_i\n";
    $totient_arr[$prime_i]=$prime_i-1;
    for (my $j = $i;$j<=$loop_max;$j++) {
      if (not $sieve[$j]) {
	my $prime_j=2*$j+1;
	$totient_arr[$prime_j]=$prime_j-1;
	my $prime_product=$prime_i*$prime_j;
	if ($prime_product<=$target) {
	  $totient_arr[$prime_product]=($prime_i-1)*($prime_j-1);
	}
      }
    }
  }
...
for (my $i=2;$i<=$target;$i++) {
  if (defined($totient_arr[$i])) {
    if (2*$i<=$target and (not defined($totient_arr[2*$i]))) {
      if ($i % 2 == 0) {
        $totient_arr[2*$i]=2*$totient_arr[$i];
      } else {
        $totient_arr[2*$i]=$totient_arr[$i];
      }
    }
  }
}
...
#    for (my $i=2;$i<=$target;$i++) {
#      if (defined($totient_arr[$i])) {
#	if ($prime*$i<=$target and (not defined($totient_arr[$prime*$i]))) {
#	  if ($i % $prime == 0) {
#	    $totient_arr[$prime*$i]=$prime*$totient_arr[$i];
#	  } else {
#	    $totient_arr[$prime*$i]=($prime-1)*$totient_arr[$i];
#	  }
#	}
#      }
#    }
...
for (my $i=2;$i*$i<=$target;$i++) {
  for (my $j=$i;$i*$j<=$target;$j++) {
    if ((not defined($totient_arr[$i*$j])) and defined($totient_arr[$j]) and defined($totient_arr[$i])) {
      my $d=gcd($i,$j);
      print "gcd($i,$j) = $d\n";
      if (defined($totient_arr[$d])) {
